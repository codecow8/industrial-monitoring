...
function TrendDesignDrawer({ onClose }) {
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="design-drawer" role="dialog" aria-modal="true" aria-labelledby="design-title"><header className="drawer-header"><div><h2 id="design-title">组件右键菜单原型 v4.1.0</h2><p>在已通过的实时趋势编辑器上，增加统一的组件右键操作入口。</p></div><button className="close-button" type="button" aria-label="关闭" onClick={onClose}><Icon name="close" /></button></header><section className="decision-block"><h3>本版验证什么</h3><p>组态工程师右键任意画布组件时，不再看到浏览器默认菜单，而是在指针附近看到工业工具风格的组件菜单。</p></section><section className="decision-block"><h3>核心操作</h3><ul><li>右键指标卡或趋势图，同时选中该组件并打开菜单。</li><li>点击“删除组件”同步更新画布和 Schema。</li><li>使用顶部撤销恢复刚删除的组件。</li><li>左键菜单外部或右键空白画布关闭菜单。</li></ul></section><section className="decision-block"><h3>明确不做</h3><p>本版不做复制、置顶置底、锁定、多选或批量操作。</p></section></aside></div>;
}
