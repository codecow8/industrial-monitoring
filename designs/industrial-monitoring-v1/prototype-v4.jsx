...
function TrendPalette({ addTrend, showToast }) {
  const items = [
...
function TrendInspector({ node, updateProp, schemaText, copySchema }) {
  ...
  return <aside className="right-panel" aria-label="属性配置"><div className="panel-title">属性配置</div><section className="inspector-section"><div className="panel-heading"><span className="section-title">基础属性</span><span className="type-code">{node.type}</span></div><div className="field-list">
    {isMetric && <div className="field"><label htmlFor="deviceName">设备名称</label><input id="deviceName" value={node.props.deviceName} onChange={(event) => updateProp("deviceName", event.target.value)} /></div>}
    <div className="field"><label htmlFor="title">{isMetric ? "指标标题" : "图表标题"}</label><input id="title" value={node.props.title} onChange={(event) => updateProp("title", event.target.value)} /></div>
    ...
    <div className="field"><label htmlFor="unit">单位</label><input id="unit" value={node.props.unit} onChange={(event) => updateProp("unit", event.target.value)} /></div>
    <div className="field"><label htmlFor="precision">小数位</label><input id="precision" type="number" min="0" max="3" value={node.props.precision} onChange={(event) => setNumber("precision", event.target.value)} /></div>
    <div className="field"><label htmlFor="threshold">告警阈值</label><input id="threshold" type="number" value={node.props.alarmThreshold} onChange={(event) => setNumber("alarmThreshold", event.target.value)} /></div>
...
function TrendDesignDrawer({ onClose }) {
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="design-drawer" role="dialog" aria-modal="true" aria-labelledby="design-title"><header className="drawer-header"><div><h2 id="design-title">实时趋势原型 v4</h2><p>在已有遥测通道上增加可编辑、可发布的单数据键趋势图。</p></div><button className="close-button" type="button" aria-label="关闭" onClick={onClose}><Icon name="close" /></button></header><section className="decision-block"><h3>本版验证什么</h3><p>第二种组件继续复用 PageSchema、发布版本、Renderer 和页面级 WebSocket，不引入历史数据库。</p></section><section className="decision-block"><h3>Trend Sample</h3><p>每秒取该秒最新值，仅保留当前运行会话最近 60 秒；断流不复制旧值，恢复后保留缺口。</p></section><section className="decision-block"><h3>核心操作</h3><ul><li>在组件栏点击“折线图”添加默认组件。</li><li>选中后可拖动、缩放和编辑数据属性。</li><li>发布后观察曲线、阈值、过期与断流缺口。</li></ul></section><section className="decision-block"><h3>明确不做</h3><p>不做历史查询、多曲线、时间范围选择、图表样式配置、复制或删除组件。</p></section></aside></div>;
}

function TrendRuntime({ published, onBack }) {
  const sequence = [68.4, 72.0, 78.5, 81.2, 83.0, 79.0, 74.0];
  const indexRef = useRef(0), timersRef = useRef([]);
  ...
  const emitNext = () => { const nextValue = sequence[indexRef.current % sequence.length]; indexRef.current += 1; appendValue(nextValue); };
  ...
  const ageSeconds = lastUpdated ? Math.max(0, Math.floor((now - lastUpdated) / 1000)) : 0;
  const recover = () => { timersRef.current.forEach(window.clearTimeout); timersRef.current = []; setReconnectStep(0); setPaused(false); setConnection("connected"); appendValue(68.4); };
  ...
  return <main className="runtime" data-screen-label="运行态趋势预览"><header className="runtime-topbar"><div className="runtime-title"><strong>{page?.name ?? "运行态"}</strong><span>{published ? `发布版本 · v${published.version}` : "尚未发布"}</span></div><div className="runtime-actions"><span className={`runtime-status ${connection} ${freshness}`}>{connectionText}</span><button className="btn btn-secondary" type="button" onClick={onBack}><span className="button-content"><Icon name="arrowLeft" />返回编辑器</span></button></div></header><section className="runtime-canvas">
    {page ? page.components.map((node) => <div key={node.id} className="runtime-component" style={{ left: node.position.x, top: node.position.y, width: node.size.width, height: node.size.height }}>{node.type === "metric-card" ? <MetricCard schema={node} value={value} freshness={freshness} ageSeconds={ageSeconds} /> : <TrendChart node={node} samples={samples} now={now} freshness={freshness} ageSeconds={ageSeconds} />}</div>) : <div className="runtime-empty" role="status"><strong>页面尚未发布</strong><span>返回编辑器发布后才会显示页面内容。</span></div>}
    {page && <div className="prototype-controls" aria-label="原型演示控制"><strong>原型演示</strong><button className={paused ? "active" : ""} type="button" onClick={() => setPaused((current) => !current)}>{paused ? "恢复上报" : "暂停上报"}</button><button type="button" onClick={() => { setPaused(true); setConnection("connected"); appendValue(83.0); }}>触发告警</button><button type="button" onClick={disconnect}>模拟断线</button><button type="button" onClick={recover}>恢复正常</button></div>}<div className="runtime-note">每秒记录一个 Trend Sample；暂停 5 秒可观察过期，恢复后保留断流缺口。</div></section></main>;
}
...
function TrendEditor({ pageSchema, setPageSchema, onPreview, onPublish, publishedVersion }) {
  ...
  const [selectedId, setSelectedId] = useState(pageSchema.components[0].id), [toast, setToast] = useState(""), [drawerOpen, setDrawerOpen] = useState(false), [savedAt, setSavedAt] = useState("已自动保存");
  ...
  const movePointer = (event) => {
    ...
    if (state.mode === "resize") {
      ...
      updateNode(state.id, (node) => {
        ...
        const minHeight = node.type === "trend-chart" ? 240 : 200;
...
function TrendPrototypeApp() {
  ...
  const [pageSchema, setPageSchema] = useState({ version: "1.0.0", id: "demo", name: "冷却系统监控", canvas: { width: 1440, height: 900, background: "#0e1d2b" }, components: [{ id: "metric-pump-01", type: "metric-card", position: { x: 275, y: 180 }, size: { width: 300, height: 214 }, props: { deviceName: "1号冷却泵", title: "出口温度", dataKey: "pump1.outlet_temp", unit: "°C", precision: 1, alarmThreshold: 80 } }] });
  ...
  return mode === "runtime" ? <TrendRuntime published={published} onBack={() => setMode("editor")} /> : <TrendEditor pageSchema={pageSchema} setPageSchema={setPageSchema} onPreview={() => setMode("runtime")} onPublish={publish} publishedVersion={published?.version ?? 0} />;
}
