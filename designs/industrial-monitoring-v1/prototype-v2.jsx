    ...
    <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <aside className="design-drawer" role="dialog" aria-modal="true" aria-labelledby="design-title">
        <header className="drawer-header">
          <div>
            <h2 id="design-title">发布版本原型 v2</h2>
            <p>这份原型补充草稿、已发布版本和运行态之间的隔离关系，不包含真实业务代码。</p>
          </div>
        ...
        <section className="decision-block">
          <h3>本版验证什么</h3>
          <p>草稿只有在用户明确点击“发布版本”后才生成不可变版本；运行态只读取最近一次发布结果。</p>
        </section>
        <section className="decision-block">
          <h3>核心操作</h3>
          <ul>
            ...
            <li>保存草稿不会改变运行态。</li>
            <li>发布后生成 v1、v2 等不可变版本。</li>
            <li>运行态明确展示当前发布版本号。</li>
          </ul>
        </section>
        <section className="decision-block">
          <h3>草稿与发布版本</h3>
          <p>编辑器始终操作草稿；发布动作复制当前草稿形成版本快照。发布后继续编辑，不会污染运行态。</p>
        </section>
        <section className="decision-block">
          <h3>下一阶段</h3>
          <p>下一步只设计 WebSocket 的连接中、已连接、断开、重连和数据过期状态。</p>
        </section>
      </aside>
    </div>
  );
  ...
  return (
    <main className="runtime" data-screen-label="运行态预览">
      ...
      <section className="runtime-canvas">
        ...
        <div className="runtime-note">运行态只读取最近一次发布的版本；保存草稿不会改变这里的内容。</div>
      </section>
    </main>
  );
}
