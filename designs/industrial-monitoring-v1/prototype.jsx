...
function AlarmList({ node, alarms = [], freshness = "fresh", ageSeconds = 0, example = false, onPointerDown }) {
...
function TrendDesignDrawer({ onClose }) {
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="design-drawer" role="dialog" aria-modal="true" aria-labelledby="design-title"><header className="drawer-header"><div><h2 id="design-title">活动告警列表原型</h2><p>聚合当前页面仍未恢复的 Alarm Condition，同时保留 Data Freshness 的可信度提示。</p></div><button className="close-button" type="button" aria-label="关闭" onClick={onClose}><Icon name="close" /></button></header><section className="decision-block"><h3>本次验证什么</h3><p>阈值越界和设备故障进入同一个只读列表，相同条件合并，明确恢复后自动移除。</p></section><section className="decision-block"><h3>排序与状态</h3><ul><li>设备故障排在阈值越界之前。</li><li>等待数据与当前无告警使用不同空状态。</li><li>数据过期时保留告警并降低强调。</li></ul></section><section className="decision-block"><h3>核心操作</h3><ul><li>从组件栏添加一个告警列表。</li><li>配置组件标题并发布。</li><li>在运行态触发阈值告警和设备故障。</li><li>暂停上报后检查告警数据过期。</li><li>恢复正常后检查空状态。</li></ul></section><section className="decision-block"><h3>明确不做</h3><p>本次不做告警确认、历史、筛选、严重级别、后端持久化或 AI 诊断。</p></section></aside></div>;
}
